...
const API_URL = 'http://localhost:3001/api'; // Change to your server IP/domain
...
// ── PRODUCTS API ──────────────────────────────────────────
const ProductsAPI = {
  getAll: (cat) => apiFetch('/products' + (cat ? `?cat=${encodeURIComponent(cat)}` : '')),
...
const WilayasAPI = {
  ...
  update: (id, data) => apiFetch('/wilayas/' + id, { method: 'PUT', body: JSON.stringify(data) }),
};

// ── RATINGS API ───────────────────────────────────────────
const RatingsAPI = {
  submit: (rating, comment, order_ref) => apiFetch('/ratings', { method: 'POST', body: JSON.stringify({ rating, comment, order_ref }) }),
